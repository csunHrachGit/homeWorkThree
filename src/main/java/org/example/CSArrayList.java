        ...
        extends AbstractList<E>
{
    ...
    public boolean addAll(int index, Collection<? extends E> c) {
        ...
        int numNew = c.size();
        ensureCapacity(size + numNew); // Grow capacity once to fit all new elements

        // Shift existing elements to make room for the new elements
        for (int i = size - 1; i >= index; i--) {
            theData[i + numNew] = theData[i];
        }

        // Insert new elements from the collection
        int i = index;
        for (E element : c) {
            theData[i++] = element;
        }
